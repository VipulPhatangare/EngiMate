...
// http://localhost:5050/api/collegePredictor
...
async function getRankFromPercentile(percentile) {
    ...
    try {
        const { data, error } = await supabase
            .from('average_merit_list_2025_26')
            .select('rank')
...
async function getColleges(formData) {
    ...
    let table_name = `MH_2025_26_CAP_${formData.round}`;

    // console.log("Generated SQL:", caste_column);
    // console.log("caste condition", caste_condition);

    try {
        const { data, error } = await supabase.rpc('collegepredictorlist', {
...
function college_filter(colleges, formData){
    ...
    colleges.forEach(element => {
        // Extract GOPEN rank for probability calculation
        let gopenRank = 0;
        if (element.gopen && element.gopen !== '0 (0)') {
            const match = element.gopen.match(/^(\d+)/);
            if (match) {
                gopenRank = parseInt(match[1]);
            }
        }

        // Extract LOPEN rank for probability calculation (for female students)
        let lopenRank = 0;
        if (element.lopen && element.lopen !== '0 (0)') {
            const match = element.lopen.match(/^(\d+)/);
            if (match) {
                lopenRank = parseInt(match[1]);
            }
        }
        
        // Add probability with gender and both ranks
        element.probability = calculateProbability(gopenRank, lopenRank, formData.gender);
...
router.post('/collegePredictorList', async(req,res)=>{
...
router.post('/collegePredictorListAI', async(req,res)=>{

    const formData = req.body;
    // console.log(formData);
    clear_new_data_function();

    new_data_of_student.maxAIper = 100.0000;
    new_data_of_student.minAIper = formData.percentile - 10.0000;
    if(formData.percentile < 97.0000){
        new_data_of_student.maxAIper = formData.percentile + 3.0000;
    }
    ...
    caste_column = `
        r.cap_${formData.round}_rank AS airank, 
        r.cap_${formData.round}_per AS aiper
    `;
    
    caste_condition = `
        r.cap_${formData.round}_per != 0 AND r.cap_${formData.round}_per BETWEEN ${new_data_of_student.minAIper} AND ${new_data_of_student.maxAIper}
    `;

    try {
        const { data, error } = await supabase.rpc('collegepredictorlistai', {
            caste_column: caste_column,
            caste_condition: caste_condition
        });
        ...
        if(error){
            ...
            // console.log(data)
            const colleges =  college_filter_ai(data, formData);
